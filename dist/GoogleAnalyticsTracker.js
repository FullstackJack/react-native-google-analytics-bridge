...
class GoogleAnalyticsTracker {
    ...
    trackScreenView(screenName, payload = null) {
        ...
        NativeBridges_1.AnalyticsBridge.trackScreenView(this.id, screenName, payload);
    }
    /**
    ...
    trackEvent(category, action, eventMetadata, payload = null) {
        this.transformPayload(payload);
        NativeBridges_1.AnalyticsBridge.trackEvent(this.id, category, action, (eventMetadata && eventMetadata.label ? eventMetadata.label : null), (eventMetadata && eventMetadata.value != null ? eventMetadata.value.toString() : null), payload);
    }
    /**
    ...
    trackTiming(category, interval, timingMetadata, payload = null) {
        this.transformPayload(payload);
        NativeBridges_1.AnalyticsBridge.trackTiming(this.id, category, interval, timingMetadata.name, timingMetadata.label, payload);
    }
    /**
     * Track an exception
     * @example
    ...
    trackException(error, fatal = false, payload = null) {
        this.transformPayload(payload);
        NativeBridges_1.AnalyticsBridge.trackException(this.id, error, fatal, payload);
    }
    /**
    ...
    trackSocialInteraction(network, action, targetUrl, payload) {
        this.transformPayload(payload);
        NativeBridges_1.AnalyticsBridge.trackSocialInteraction(this.id, network, action, targetUrl, payload);
    }
    /**
     ...
     */
    setUser(userId) {
        NativeBridges_1.AnalyticsBridge.setUser(this.id, userId);
    }
    /**
     ...
     */
    setClient(clientId) {
        NativeBridges_1.AnalyticsBridge.setClient(this.id, clientId);
    }
    /**
     ...
     */
    getClientId() {
        return NativeBridges_1.AnalyticsBridge.getClientId(this.id);
    }
    /**
    ...
    allowIDFA(enabled = true) {
        NativeBridges_1.AnalyticsBridge.allowIDFA(this.id, enabled);
    }
    /**
     ...
     */
    setAppName(appName) {
        NativeBridges_1.AnalyticsBridge.setAppName(this.id, appName);
    }
    /**
    ...
    setAppVersion(appVersion) {
        NativeBridges_1.AnalyticsBridge.setAppVersion(this.id, appVersion);
    }
    /**
    ...
    setAnonymizeIp(enabled) {
        NativeBridges_1.AnalyticsBridge.setAnonymizeIp(this.id, enabled);
    }
    /**
    ...
    setSamplingRate(sampleRatio) {
        NativeBridges_1.AnalyticsBridge.setSamplingRate(this.id, sampleRatio);
    }
    /**
    ...
    setCurrency(currencyCode) {
        NativeBridges_1.AnalyticsBridge.setCurrency(this.id, currencyCode);
    }
    /**
    ...
    setTrackUncaughtExceptions(enabled) {
        NativeBridges_1.AnalyticsBridge.setTrackUncaughtExceptions(this.id, enabled);
    }
    /**
     ...
     */
    dispatch() {
        return NativeBridges_1.AnalyticsBridge.dispatch();
    }
    /**
    ...
    dispatchWithTimeout(timeout = -1) {
        ...
        return Promise.race([
            NativeBridges_1.AnalyticsBridge.dispatch(),
            withTimeout(timeout)
        ]).then(result => {
            if (timer) {
                clearTimeout(timer);
